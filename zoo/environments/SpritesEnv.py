...
class SpritesEnv(EnvInterface):
    ...
    def __init__(self, repeats, images_archive, max_episode_length, difficulty, **_):
        """
        Constructor (compatible with OpenAI gym environment)
        :param repeats: the number of time each action is repeated in the environment
        :param images_archive: the location of the archive containing the d_sprites images
    ...
    def current_frame(self):
        ...
        image = torch.from_numpy(np.repeat(image, 3, 2)).type(torch.float32)
        return image.to(Device.get())
