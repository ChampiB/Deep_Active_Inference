...
class CHMM(AgentInterface):
    ...
    def __init__(
        self, name, encoder, decoder, transition, critic, discount_factor, n_steps_beta_reset, beta, efe_lr, vfe_lr,
        beta_starting_step, beta_rate, queue_capacity, n_steps_between_synchro, tensorboard_dir, checkpoint_dir,
        reward_coefficient, g_value, action_selection, n_states, image_shape, n_actions=4, steps_done=0,
        efe_loss_update_encoder=False, verbose=False, **_
    ):
        """
        Constructor
    ...
    def learn(self, config):
        """
        Perform on step of gradient descent on the encoder, decoder, transition and critic networks
        :param config: the hydra configuration
        """
    ...
    def compute_efe_loss(self, config, obs, actions, next_obs, done, rewards):
        ...
        immediate_gval = self.reward_coefficient * rewards + info_gain
