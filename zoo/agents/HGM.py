...
class HGM(AgentInterface):
    """
    Implement a Hierarchical Gaussian Mixture taking random each action.
    """
    ...
    def learn_sub_gm(self, node, debug=True, verbose=False):
        ...
        for state in range(self.n_states):
            ...
            sub_gm = GM(n_states=self.n_states, dataset_size=len(x), n_observations=self.n_observations)
            sub_gm.x = x
