...
class CartPoleEncoder(nn.Module):
    ...
    def __init__(self, n_states, input_size, **_):
        """
        Constructor.
        :param n_states: the number of components of the Gaussian over latent variables.
...
#
# Class implementing a convolutional encoder for 64 by 64 images, which predicts the critic output as well.
#
class CriticalConvEncoder64(nn.Module):

    def __init__(self, n_actions, n_states, image_shape, **_):
        """
        Constructor.
    ...
    def forward(self, x):
        ...
        """
        x = x.permute(0, 3, 1, 2)
        hidden_states = self.__conv_net(x)
        return self.__linear_net(hidden_states), self.__critic_net(hidden_states)
